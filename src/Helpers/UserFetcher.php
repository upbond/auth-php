...
class UserFetcher
{
     ...
     */
    public function __construct(string $token_issuer, CacheInterface $cache = null, array $guzzleOptions = [])
    {
        if ($cache === null) {
     ...
     * @return mixed|null
     */
    public function getUser(array $apiuser, string $jwt, $use_cache = true)
    {
        //jti unique identifier for this token
        $cache_key = $apiuser['jti'];
        ...
        try {
            $user = $this->requestUser($jwt);
        } catch (\Throwable $th) {
            throw new InvalidTokenException('unauthorized');
        }

        $user = array_merge($apiuser, $user);

        $this->cache->set($cache_key, $user, self::CACHE_TTL);
     ...
     */
    protected function requestUser(string $access_token) 
    {
        $options = array_merge( $this->guzzleOptions, [ 'base_uri' => $this->token_issuer ] );
